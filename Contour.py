...
# Output folder for contour-annotated images
OUTPUT_DIR = os.path.join(BASE_DIR, "output/contours_only")
...
for fname in os.listdir(DEFECTS_DIR):
    ...
    test_color = cv2.imread(os.path.join(DEFECTS_DIR, fname))  # for drawing
    ...
    for cnt in contours:
        ...
        if w * h > 50:
            ...
            cv2.putText(test_color, "Defect", (x, y - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

    out_path = os.path.join(OUTPUT_DIR, f"contours_{fname}")
    cv2.imwrite(out_path, test_color)
    print(f"Saved annotated image: {out_path}")

print("\nContour annotation completed! Results saved in:", OUTPUT_DIR)
