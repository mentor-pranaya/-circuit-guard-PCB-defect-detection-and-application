...
import numpy as np
from skimage.metrics import structural_similarity as ssim  # ✅ SSIM for similarity
...
REF_PCB_FOLDER = os.path.join(BASE_FOLDER, "PCB_USED")  # 📌 store all golden PCBs here
...
# ==========================================
# 2️⃣ Find Best Matching Reference PCB
# ==========================================
def find_best_reference(defect_img_path, ref_folder):
    ...
    return best_ref_path, best_score
...
# 6️⃣ Full Pipeline
# ==========================================
def process_images(defect_img_path):
    base_name = os.path.basename(defect_img_path)

    # 🔍 Auto-select best reference PCB
    best_ref, score = find_best_reference(defect_img_path, REF_PCB_FOLDER)
    if best_ref is None:
        raise ValueError("❌ No matching reference PCB found.")

    st.write(f"✅ Matched Reference PCB: **{os.path.basename(best_ref)}** (SSIM: {score:.4f})")

    mask_save_path = os.path.join(SUBTRACTED_SAVE, f"{os.path.splitext(base_name)[0]}_mask.png")

    defect_color, mask = subtract_images(defect_img_path, best_ref, save_mask_path=mask_save_path)
...
st.write("Upload a **defected PCB**. The system will auto-match the correct golden PCB and detect issues.")
...
if defect_file:
    ...
    if st.button("Run Detection"):
        annotated_img, preds, annotated_save_path = process_images(defect_path)
